...
fn_SetStat =
{
	...
	//	if(_varName == 'backpackPlayer') then {player addBackpack _varValue;};
		//if(_varName == 'misspos')then {misspos= _varValue};
		if(_varName == 'enmoral')then {enemymoral= _varValue};
		if(_varName == 'date')then {setDate _varValue};
		if(_varName == 'overcast')then {0 setovercast _varValue};
	}
	else
	{
