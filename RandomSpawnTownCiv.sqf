...
/*
null=[1,true,true,[150,200,300],Tgren,[75,100,150]] execVM "RandomSpawnTownCiv.sqf";
...
_positionH = getPos civ1;
_house = nearestBuilding civ1;
_x = 0;

while { format ["%1", _house buildingPos _x] != "[0,0,0]" } do {_x = _x + 1};
_x = _x - 1;

civ1 setPos (_house buildingPos (random _x));
civ2 setPos (_house buildingPos (random _x));

	} else{
					if (_NonMuovere ==1) then {}
				else {
[_mygroup, _mygroup, _Raggio, 7, "MOVE", "CARELESS", "WHITE", "LIMITED", "STAG COLUMN", "civ1 spawn CBA_fnc_searchNearby", [3,6,9]] call CBA_fnc_taskPatrol;
					};
					};
			_Origine=_Origine+1;
			sleep 1;
