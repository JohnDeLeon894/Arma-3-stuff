...
_commandingUnit = _heli getVariable "NORRN_H_commandingUnit";
...
[_new_heli] execVM "scripts\b2_heliDoors.sqf";
...
for [{ _loop = 0 },{ _loop < count  _crew},{ _loop = _loop + 1}] do
{
	...
	_unit = _typeNew createUnit [[(getPos _base select 0) + 2 +(random 3),(getPos _base select 1) + 2 +(random 3),0], _group];
	sleep 1;
};
sleep 2;
// Crew board new heli
...
_new_heli setVariable ["NORRN_H_commandingUnit", _commandingUnit, true];
_new_heli setVariable ["NORRN_H_destChosen", false, true];
_new_heli setVariable ["NORRN_aerialTaxiDestroyed", false, true];
_new_heli setVariable ["NORRN_heloGoto_cancel", false, true];
_new_heli setVariable ["NORRN_aerialTaxiRTB", false, true];
_commandingUnit setVariable ["NORRN_taxiHeli", _new_heli, true];
sleep 1;
