...
sleep 5;
while {_heli distance _heliH > 200 && alive _heli && canMove _heli && !(_heli getVariable "NORRN_heloGoto_cancel")} do 
{	
	_heli doMove _wp_target;
	sleep 5;
};
if (canMove _heli && alive _heli && !(_heli getVariable "NORRN_heloGoto_cancel")) then 
{
...
while {_heli distance _heliH > 100 && canMove _heli && !(_heli getVariable "NORRN_heloGoto_cancel")} do 
{
	_heli doMove _wp_target;
	sleep 5;
};
if (canMove _heli) then {_heli land "LAND"};
while {_heli distance _heliH > 20 && (getPos _heli select 2) > 10 && canMove _heli && !(_heli getVariable "NORRN_heloGoto_cancel")} do 
{
	_heli doMove (getPos _heliH);
	sleep 5;
};
if (canMove _heli && !(_heli getVariable "NORRN_heloGoto_cancel")) then
{ 	
	if (canMove _heli) then {_heli land "LAND"};
...
while {(getPos _heli) select 2 < 20 && canMove _heli && alive _heli && !(_heli getVariable "NORRN_heloGoto_cancel")} do {sleep 0.1};
