...
{	
	if ( format ["%1", (player getVariable "NORRN_taxiHeli")] != "<null>") then {_gotoHeli = player getVariable "NORRN_taxiHeli"};
...
{	
	...
	if (count _cargo_heli > 0 && !(_heli getVariable "NORRN_H_destChosen") && !(_heli getVariable "NORRN_aerialTaxiRTB")) then
	{	
		_isplayer_cargo = [];
	...
	{
		{if (!(_x in _crew_heli) || (_heli getVariable "NORRN_H_destChosen")) then {_oldLeader = _oldLeader - [_x]}} forEach _oldLeader;
	};
	sleep 1;
};